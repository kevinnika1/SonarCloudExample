...
def run(username,password,website,project):
    ...
    response = requests.post(url_delete_option, auth=HTTPBasicAuth('ntern5kn','fipB8lX-!'))
    dict_data = xmltodict.parse(response.content) #even though these arent needed , leave them in incase of future debugging 
    print(dict_data)
    ...
    response = requests.post(url_delete_auth, auth=HTTPBasicAuth('ntern5kn','fipB8lX-!'))
    dict_data = xmltodict.parse(response.content) #even though these arent needed , leave them in incase of future debugging 
    print(dict_data)
