...
for x in issuesJSON['issues']:
    if (x['project']== project):
        thisdict= {}
        thisdict['file']=x['component'].replace(project+":", '')#thisdict['file']=x['component'].replace(project+":", '/')
        thisdict['title']=x.get('component')
        thisdict['message']=x.get('message')
        thisdict['annotation_level']=x.get('severity')#thisdict['annotation_level']=x.get('severity')
        for y in x['flows']:
